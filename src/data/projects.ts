...
複数の技術要素を統合した検証環境の構築からデータ分析までを一貫して行い、情報処理学会全国大会にて学生奨励賞を受賞しています。`,
    ...
    repositoryUrl: "https://github.com/Garden-Tree/motorcycle-hmd-study"
  },
  {
...
自身のライダーとしての経験から、ガソリンスタンドで即座に記録が完了するUIを設計。ユーザーは「写真を撮るだけ」で燃費の推移や月間コストを把握できる仕組みを構築しました。`,
  ...
  {
    title: "Raspberry Pi等を用いたIoTデバイス開発とデータフィード基盤の運用",
    slug: "aircraft-ads-b-feeder",
    shortDescription: "ArduinoやRaspberry Piを活用し、課題解決のためのIoTデバイス制作や、航空機情報(ADS-B)の受信・データ連携システムを構築・運用しています。",
    longDescription: `物理世界（ハードウェア）とデジタル空間を繋ぐアプローチとして、エッジデバイスを用いたIoTシステムの設計・実装・運用を行っています。

日常の課題を解決するIoTデバイスの実装
センサーを用いてゴミの量を検知し通知する「スマートゴミ箱」や、ネットワーク経由で赤外線を送信しスマートフォンから家電を操作する「スマートリモコン」をArduinoやRaspberry Piを用いて実装。ハードウェアの制御機構や通信プロトコルの仕組みをハンズオンで学んでいます。

SDRとRaspberry Piを用いたADS-B受信システムの構築
SDR（ソフトウェア定義無線）チューナーとRaspberry Piを組み合わせ、近隣を飛行する航空機のADS-B信号を受信するシステムを構築しました。既存のデコードプログラムを適切に設定し、電波情報をデジタルデータに変換しています。
...
取得した航空機のデータを、Flightradar24などの共有サイトへ安定してフィードし続けています。Raspberry Piという限られたリソース・電源環境下において、熱対策やLinuxの安定稼働といった運用知見を蓄積しています。`,
    ...
    repositoryUrl: "https://github.com/Garden-Tree/aircraft-ads-b-feeder"
  },
  {
    ...
    repositoryUrl: "https://gadelog.com/"
  },
  {
    title: "スクールバス運行最適化提案",
